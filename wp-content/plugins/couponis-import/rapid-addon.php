				...
				foreach ($this->fields as $field_slug => $field_params) {
					...
					switch ($field_params['type']) {
						...
						case 'image':
							...
							$filename = '';
							$featured_as_store = false;

							if( $download == 'yes' && $field_slug != 'store_image' ){

								if( $field_slug == 'coupon_image' && $parsedData['coupon_image'][$index] == $parsedData['store_image'][$index] ){
									$featured_as_store = true;
								}

								if( !$featured_as_store ){
									if( $field_slug == 'coupon_image' ){
										$filename = 'coupon-image-'.$post_id;	
									}
									else if( $field_slug == 'coupon_printable' ){
										$filename = 'coupon-printable-'.$post_id;	
									}

									$bn  = wp_all_import_sanitize_filename( urldecode( basename( $image_url_or_path ) ) );
									$img_ext = pmxi_getExtensionFromStr( $image_url_or_path );
									$default_extension = pmxi_getExtension( $bn );
									if ( $img_ext == "" ) $img_ext = pmxi_get_remote_image_ext( $image_url_or_path );
									
									// generate local file name
									$filename = apply_filters( "wp_all_import_image_filename", $filename.( "" != $img_ext ? '.' . $img_ext : '') );
								}
							}
							
							if( $download == 'no' || ( $download == 'yes' && $field_slug != 'store_image' && !$featured_as_store ) ){
								$uploaded_image = PMXI_API::upload_image($post_id, $image_url_or_path, $download, $importData['logger'], true, $filename);
							}
							else{
								$uploaded_image = '';
							}
							...
							$data[$field_slug] = array(
								...
								"download" => $download,
								"featured_as_store" => $featured_as_store
							);
