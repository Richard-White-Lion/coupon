...
function couponis_get_coupon_image( $image_size ){
	...
	if( $show_featured ){ 
		...
		if( !empty( $square_image_id ) ){
			...
			echo '<a '.couponis_get_coupon_link().' class="coupon-image"><img alt="'.get_the_title().'" src="'. $square_image_url .'"/></a>';
		} else {
			echo '<a '.couponis_get_coupon_link().' class="coupon-image test">'.get_the_post_thumbnail( get_the_ID(), $image_size, $image_attr).'</a>';
		}		
	}
}
}
...
/*
Get vendor store link
*/

if( !function_exists('couponis_get_vendor_store_link') ){
	function couponis_get_vendor_store_link(){
		$store = get_the_terms( get_the_ID(), 'coupon-store' );
		if( $store ){
			if( is_array( $store ) ){
				$store = array_shift( $store );
			...
			$link = get_term_link( $store );
			return '<a href="'.$link.'">'.esc_html( $store->name ).'</a>';
		}
		return '';
	}
}
...
if( !function_exists('couponis_get_vendor_name') ){
	function couponis_get_vendor_name(){
		$store = get_the_terms( get_the_ID(), 'coupon-store' );
		if( $store ){
			if( is_array( $store ) ){
				$store = array_shift( $store );
