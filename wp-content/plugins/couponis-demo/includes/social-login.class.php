...
class Couponis_Social_Login{
	...
	public function start_social_login(){
		foreach( $this->networks as $network => &$data ){
			$id = couponis_get_option( $network.'_app_id' );
			$secret = couponis_get_option( $network.'_app_secret' );
			$data['data'] = array(
				'id'		=> $id,
				'secret'	=> $secret
			);
	...
	public function load_scripts(){
		wp_localize_script( 'couponis-custom', 'couponis_sc', array(
			'facebook' 	=> site_url( 'index.php?couponis-connect=facebook' ),
			'twitter' 	=> site_url( 'index.php?couponis-connect=twitter' ),
			'google' 	=> site_url( 'index.php?couponis-connect=google' )
		));
	}
	...
	public function show_buttons(){
		if( $this->social_enabled == true ){
			...
			<div class="text-center">
				<label><?php esc_html_e( 'Sign In With', 'couponis' ) ?></label>
			</div>
	...
	public function facebook_connect(){
		wp_redirect( 'https://graph.facebook.com/oauth/authorize?scope=email&client_id='.esc_attr( $this->networks['facebook']['data']['id'] ).'&redirect_uri='.site_url('index.php?couponis-callback=facebook') );
		die();
	}
	...
	public function facebook_callback(){
		if( isset( $_GET['code'] ) ){
			$response = $this->_http_facebook( 'oauth/access_token', array(
				...
				'redirect_uri'	=> site_url( 'index.php?couponis-callback=facebook' ),
			...
			if( !empty( $response->access_token ) ){
				...
				if( !empty( $user->id ) ){
					...
					esc_html_e( 'Could not retrieve user data', 'couponis' );
				}
			}
			else{
				esc_html_e( 'Could not generate access_token', 'couponis' );
			}
		} 
		else {
	...
	public function twitter_connect(){
		$response = $this->_http_twitter( 'oauth/request_token', array(
			'oauth_callback' => site_url('/index.php?couponis-callback=twitter')
		));
		...
		if( !empty( $response['oauth_callback_confirmed'] ) && $response['oauth_callback_confirmed'] == true ){
			...
			esc_html_e( 'Could not generate oauth_token since credentials are invalid', 'couponis' );
		}
		die();
	}
	...
	public function twitter_callback(){
		if( isset( $_GET['oauth_token'] ) && isset( $_GET['oauth_verifier'] ) ){
			...
			if( !empty( $response['oauth_token'] ) && !empty( $response['oauth_token_secret'] ) ){
				...
				esc_html_e( 'Could not generate oauth_token_secret since credentials are invalid', 'couponis' );
			}

		} 
		else {
			esc_html_e( 'Login error', 'couponis' );
		}
		die();
	}
	...
	public function google_connect(){
		$data = array(
			...
			'redirect_uri'		=> site_url('index.php?couponis-callback=google'),
	...
	public function google_callback(){
		if( isset( $_GET['code'] ) ){
			...
			if( !empty( $response->access_token ) ){
				...
				esc_html_e( 'Could not retrieve access_token', 'couponis' );	
			}
		} 
		else {
			esc_html_e( 'Login error', 'couponis' );
